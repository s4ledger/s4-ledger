...
// Hybrid auto-play with manual skip/click-through
...
var _steps = [
    {
        id: 'welcome', title: 'Welcome to S4 Ledger', icon: 'fas fa-shield-halved', cat: 'Overview', dur: 10000,
        narr: 'Welcome to S4 Ledger — the integrity layer for defense logistics. This walkthrough shows how the platform helps you track parts, verify records, and prove compliance under audit. Every step uses realistic defense data.',
        mock: function() {
    ...
    {
        id: 'anchor', title: 'Anchor a Record', icon: 'fas fa-anchor', cat: 'Core', dur: 12000,
        narr: 'Start by anchoring a logistics record. Paste or type any supply chain data — the platform creates a SHA-256 fingerprint and writes it to the XRP Ledger in under 3 seconds. This record is now tamper-proof.',
        mock: function() {
            return '<div class="wt-mock-tool"><div class="wt-mock-tool-head"><i class="fas fa-anchor"></i> Anchor Record</div>'
                ...
                + '<button class="wt-mock-btn wt-mock-btn-primary" id="wtAnchorBtn" style="opacity:0.5;pointer-events:none;">Anchor to XRPL <i class="fas fa-arrow-right"></i></button>'
                + '</div>';
        },
        onEnter: function() {
    ...
    {
        id: 'confirmation', title: 'Blockchain Confirmation', icon: 'fas fa-check-circle', cat: 'Core', dur: 10000,
        narr: 'In under 3 seconds, the record is anchored to the XRP Ledger. You receive a unique transaction hash and a direct link to verify it on the blockchain explorer — permanent, public, tamper-proof.',
        mock: function() {
            return '<div class="wt-mock-tool"><div class="wt-mock-tool-head" style="color:var(--green);"><i class="fas fa-check-circle"></i> Record Anchored</div>'
                ...
                + '<div class="wt-mock-result-row"><span class="wt-mock-label">Ledger</span><span style="color:var(--accent);">XRPL Mainnet #84,291,037</span></div>'
                + '<div class="wt-mock-result-row"><span class="wt-mock-label">Time</span><span>2.4 seconds</span></div>'
                + '<div class="wt-mock-result-row"><span class="wt-mock-label">Cost</span><span>0.01 SLS ($0.0001)</span></div>'
                + '<div class="wt-mock-result-row"><span class="wt-mock-label">SHA-256</span><span class="wt-mock-hash" id="wtHashReveal"></span></div>'
                + '</div>'
                + '<a class="wt-mock-link" href="javascript:void(0)"><i class="fas fa-external-link-alt"></i> View on XRPL Explorer</a>'
                + '</div>';
        },
        onEnter: function() {
            var el = document.getElementById('wtHashReveal');
            if (el) _typeWriter(el, 'e3b0c44298fc1c149afb...b855b4e04f1dceef71e23f0c208', 18);
        }
    },
    {
        id: 'verify', title: 'Verify a Record', icon: 'fas fa-check-double', cat: 'Core', dur: 10000,
        narr: 'Verification is the other side of the coin. Paste the original record and the transaction ID — the platform recomputes the hash and compares it against the blockchain. Character-exact matching proves nothing was altered.',
        mock: function() {
    ...
    {
        id: 'tx-log', title: 'Transaction Log', icon: 'fas fa-list', cat: 'Core', dur: 8000,
        narr: 'Every anchored record appears in your transaction log with the timestamp, record type, hash, and a direct link to the blockchain. Export the full log as a spreadsheet for audit preparation.',
        mock: function() {
    ...
    {
        id: 'gap-analysis', title: 'Gap Analysis', icon: 'fas fa-search-plus', cat: 'Analysis', dur: 10000,
        narr: 'Upload program documents — DRL spreadsheets, tech manual indexes, PDFs. The Gap Analysis tool scans them against requirements, scores your coverage, and tells you exactly what is missing and what to fix first.',
        mock: function() {
        ...
        onEnter: function() { _animateNumber(document.querySelector('#wtGapScore span'), 0, 78, 1500, '%'); }
    },
    {
        id: 'dmsms', title: 'DMSMS Tracker', icon: 'fas fa-microchip', cat: 'Analysis', dur: 10000,
        narr: 'Enter part numbers to check obsolescence status instantly. The DMSMS Tracker flags parts that are discontinued, at risk, or have known alternatives — replacing days of manual GIDEP research with a 2-second lookup.',
        mock: function() {
    ...
    {
        id: 'readiness', title: 'Readiness Calculator', icon: 'fas fa-calculator', cat: 'Analysis', dur: 10000,
        narr: 'Input equipment reliability parameters — MTBF, MTTR, and administrative delay — to calculate operational availability using standard Department of Defense formulas. Results are instant and briefing-ready.',
        mock: function() {
    ...
    {
        id: 'compliance', title: 'Compliance Scorecard', icon: 'fas fa-clipboard-check', cat: 'Compliance', dur: 10000,
        narr: 'See your compliance posture at a glance. The scorecard evaluates your program against CMMC Level 2, NIST 800-171, DFARS 252.204-7012, and more — generating executive-ready letter grades with specific remediation guidance.',
        mock: function() {
    ...
    {
        id: 'supply-chain', title: 'Supply Chain Risk', icon: 'fas fa-exclamation-triangle', cat: 'Supply Chain', dur: 10000,
        narr: 'Identify supply chain vulnerabilities before they become mission-critical failures. The risk engine flags single-source suppliers, geographic concentration, long lead times, and DMSMS overlap across your entire program.',
        mock: function() {
    ...
    {
        id: 'actions', title: 'Action Items', icon: 'fas fa-tasks', cat: 'Operations', dur: 8000,
        narr: 'Every analysis generates prioritized action items sorted by urgency — critical, warning, and informational. Each item includes a cost estimate and recommended fix so your team knows exactly what to do next.',
        mock: function() {
    ...
    {
        id: 'predictive', title: 'Predictive Maintenance', icon: 'fas fa-chart-line', cat: 'Analysis', dur: 10000,
    ...
    {
        id: 'lifecycle', title: 'Lifecycle Cost', icon: 'fas fa-dollar-sign', cat: 'Analysis', dur: 9000,
        narr: 'Estimate the total cost of owning and maintaining a system over its entire service life — from acquisition through sustainment and disposal. Break down costs by phase and identify the largest cost drivers.',
        mock: function() {
            return '<div class="wt-mock-tool"><div class="wt-mock-tool-head"><i class="fas fa-dollar-sign"></i> Lifecycle Cost — CVN-78 Gerald R. Ford</div>'
                ...
                + '<div class="wt-mock-cost-row"><span>Personnel</span><div class="wt-mock-cost-row"><span>Personnel</span><div class="wt-mock-cost-bar" style="width:45%;background:var(--green);"></div><span>$19.8B</span></div>'
    ...
    {
        id: 'roi', title: 'ROI Calculator', icon: 'fas fa-chart-pie', cat: 'Analysis', dur: 9000,
        narr: 'Calculate exactly how much time and money S4 Ledger saves your organization. Input your current headcount and manual process hours — the calculator generates briefing-ready ROI figures for leadership.',
        mock: function() {
        ...
        onEnter: function() {
            ...
            if (el2) { setTimeout(function() { el2.textContent = '42x'; el2.classList.add('wt-fade-in'); }, 1200); }
        }
    },
    {
        id: 'vault', title: 'Audit Vault', icon: 'fas fa-vault', cat: 'Compliance', dur: 9000,
        narr: 'Every record you anchor is automatically stored in the Audit Vault with its blockchain verification. When an auditor asks for proof, you export it in one click — no searching through email chains or SharePoint.',
        mock: function() {
    ...
    {
        id: 'doc-library', title: 'Document Library', icon: 'fas fa-book', cat: 'Documents', dur: 8000,
        narr: 'A searchable reference library of defense standards, MIL-STDs, regulations, and instructions. Your team never needs to hunt for the right document — search by number, title, or keyword.',
        mock: function() {
    ...
    {
        id: 'reports', title: 'Report Generator', icon: 'fas fa-file-alt', cat: 'Compliance', dur: 8000,
        narr: 'Generate formal audit reports automatically — stamped to the blockchain for tamper-proof storage. Choose from pre-built report templates or customize your own. Export as PDF or share a verified link.',
        mock: function() {
            return '<div class="wt-mock-tool"><div class="wt-mock-tool-head"><i class="fas fa-file-alt"></i> Report Generator</div>'
                ...
                + '<div class="wt-mock-report-item"><i class="fas fa-file-pdf" style="color:var(--red);"></i><div><div style="color:#fff;font-weight:600;">Quarterly Compliance Report</div><div style="color:var(--steel);font-size:0.72rem;">DDG-51 FY26 Q1 — 12 pages, blockchain-verified</div></div><span class="wt-mock-badge-green">Ready</span></div>'
    ...
    {
        id: 'submissions', title: 'Submissions & PTD', icon: 'fas fa-paper-plane', cat: 'Documents', dur: 9000,
        narr: 'The ILIE submission review compares vendor-provided data against your program baselines. It automatically flags discrepancies, scores data quality, and assigns severity ratings — catching errors before they become audit findings.',
        mock: function() {
    ...
    {
        id: 'sbom', title: 'SBOM Viewer', icon: 'fas fa-sitemap', cat: 'Supply Chain', dur: 8000,
    ...
    {
        id: 'gfp', title: 'GFP Tracker', icon: 'fas fa-box', cat: 'Supply Chain', dur: 8000,
        narr: 'Track Government-Furnished Property across your program — equipment, materials, and special tooling. Every custody transfer is anchored to the blockchain, giving you a complete chain of accountability.',
        mock: function() {
    ...
    {
        id: 'cdrl', title: 'CDRL Validator', icon: 'fas fa-file-check', cat: 'Documents', dur: 8000,
        narr: 'Validate Contract Data Requirements List items against your delivery schedule. The validator checks that every required deliverable is accounted for, properly formatted, and submitted on time.',
        mock: function() {
    ...
    {
        id: 'contract', title: 'Contract Extractor', icon: 'fas fa-file-contract', cat: 'Documents', dur: 8000,
    ...
    {
        id: 'provenance', title: 'Provenance Chain', icon: 'fas fa-link', cat: 'Supply Chain', dur: 9000,
        narr: 'Visualize the complete chain of custody for any tracked item — from manufacturer to end user. Every handoff is blockchain-anchored, creating an unbroken, tamper-proof record of who had what, when.',
        mock: function() {
            return '<div class="wt-mock-tool"><div class="wt-mock-tool-head"><i class="fas fa-link"></i> Provenance Chain — NSN 5340-01-587-2963</div>'
                ...
                + '<div class="wt-mock-chain-line"></div>'
                + '<div class="wt-mock-chain-node"><div class="wt-mock-chain-dot green"></div><div class="wt-mock-chain-info"><strong>Shipped to NSSA</strong><div>Norfolk Naval Shipyard</div><div class="wt-mock-chain-date">05 Feb 2026</div></div><div class="wt-mock-chain-proof"><i class="fas fa-check-circle" style="color:var(--green);"></i></div></div>'
                + '<div class="wt-mock-chain-line"></div>'
    ...
    {
        id: 'analytics', title: 'Cross-Program Analytics', icon: 'fas fa-chart-bar', cat: 'Analysis', dur: 9000,
        narr: 'Compare performance, risk, and compliance across multiple programs in a single dashboard. Identify trends, share best practices, and allocate resources where they will have the most impact.',
        mock: function() {
    ...
    {
        id: 'team', title: 'Team Management', icon: 'fas fa-users', cat: 'Operations', dur: 8000,
        narr: 'Manage your team with role-based access control. Assign capabilities per user — admins manage SLS allocations, program managers oversee deliverables, and analysts focus on their tools. Every permission change is logged.',
        mock: function() {
            return '<div class="wt-mock-tool"><div class="wt-mock-tool-head"><i class="fas fa-users"></i> Team Management — 8 Members</div>'
                + '<table class="wt-mock-table"><thead><tr><th>Name</th><th>Role</th><th>Last Active</th><th>SLS Used</th></tr></thead><tbody>'
                + '<tr><td>CDR J. Martinez</td><td><span class="wt-mock-badge-blue">Admin</span></td><td>Today</td><td>1,240</td></tr>'
                + '<tr><td>LCDR S. Thompson</td><td><span class="wt-mock-badge-green">PM</span></td><td>Today</td><td>890</td></tr>'
                + '<tr><td>LT R. Chen</td><td><span class="wt-mock-badge-yellow">Analyst</span></td><td>Yesterday</td><td>456</td></tr>'
                + '<tr><td>LT K. Davis</td><td><span class="wt-mock-badge-yellow">Analyst</span></td><td>Today</td><td>312</td></tr>'
                + '</tbody></table></div>';
        }
    },
    {
        id: 'summary', title: 'Ready to Get Started?', icon: 'fas fa-rocket', cat: 'Summary', dur: 15000,
        narr: 'That is S4 Ledger — 20 integrated tools for defense logistics, every record anchored to the XRP Ledger, every action auditable. Whether you are tracking a single part or managing an entire fleet, the platform scales with you. Sign in to start anchoring.',
        mock: function() {
            return '<div class="wt-mock-welcome">'
                ...
                + '<p style="color:var(--steel);font-size:0.88rem;margin-bottom:20px;max-width:400px;">S4 Ledger gives your team 20+ defense logistics tools backed by blockchain-verified integrity. Every record. Every handoff. Proven.</p>'
                + '<div class="wt-stat-grid">'
...
// ═══ State ═══
var _wtIdx = -1;
var _wtPlaying = false;
var _wtVoice = false;
var _wtTimer = null;
var _wtStepTimer = null;
...
function _showWTStep() {
    ...
    var playBtn = overlay.querySelector('.wt-btn-play i');
    ...
    if (playBtn) playBtn.className = _wtPlaying ? 'fas fa-pause' : 'fas fa-play';

    // Typewriter narrator text
    if (desc) {
        desc.textContent = '';
        _typeWriter(desc, step.narr, 22);
    }
    ...
    // Run onEnter animations after a short delay
    if (step.onEnter) { setTimeout(step.onEnter, 800); }
    ...
    // Auto-advance timer
    clearTimeout(_wtTimer);
    if (_wtPlaying) {
        _wtTimer = setTimeout(function() { _wtNext(); }, step.dur);
    }

    // Step progress animation
    clearInterval(_wtStepTimer);
    var stepBar = overlay.querySelector('.wt-step-progress-fill');
    if (stepBar && _wtPlaying) {
        stepBar.style.transition = 'none';
        stepBar.style.width = '0%';
        setTimeout(function() {
            stepBar.style.transition = 'width ' + (step.dur / 1000) + 's linear';
...
function _wtPrev() {
    if (_wtIdx > 0) { _wtIdx--; _showWTStep(); }
}

function _wtTogglePlay() {
    _wtPlaying = !_wtPlaying;
    var overlay = document.getElementById('s4WalkthroughOverlay');
    if (overlay) {
        var playBtn = overlay.querySelector('.wt-btn-play i');
        if (playBtn) playBtn.className = _wtPlaying ? 'fas fa-pause' : 'fas fa-play';
    ...
    if (_wtPlaying) {
        // Resume auto-advance
        var step = _steps[_wtIdx];
        if (step) { _wtTimer = setTimeout(function() { _wtNext(); }, step.dur / 2); }
        var stepBar = document.querySelector('.wt-step-progress-fill');
        if (stepBar) { stepBar.style.transition = 'width ' + (step.dur / 2000) + 's linear'; stepBar.style.width = '100%'; }
    } else {
        clearTimeout(_wtTimer);
        if ('speechSynthesis' in window) speechSynthesis.pause();
        var stepBar2 = document.querySelector('.wt-step-progress-fill');
        if (stepBar2) { var w = stepBar2.getBoundingClientRect().width / stepBar2.parentElement.getBoundingClientRect().width * 100; stepBar2.style.transition = 'none'; stepBar2.style.width = w + '%'; }
    }
}
...
function submitFeedback() {
    ...
    // Store in localStorage (Supabase integration can be added later)
    var stored = [];
...
// Thumbs up/down on tool results
...
// Init feedback drawer on DOM ready
...
window._wtTogglePlay = _wtTogglePlay;
