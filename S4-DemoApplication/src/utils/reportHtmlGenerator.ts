...
function getRowAnalysis(row: DRLRow, anchors: Record<string, AnchorRecord>): string {
  ...
  if (row.status === 'green') {
    return `${req.requiredVersion} ${req.requiredRevision} received via ${req.submittalMethod} on ${row.actualSubmissionDate}. Deliverable accepted per Attachment J-2, ${req.contractRef}. ${anchors[row.id] ? 'Hash verified and sealed to XRPL ledger.' : 'Pending ledger seal.'} No corrective action required.`
  }
  if (row.status === 'red') {
    ...
    return `Submitted ${late} calendar days late (${row.actualSubmissionDate} vs due ${req.contractDue}). Per ${req.block}, this exceeds allowable variance and constitutes an Attachment J-2 delinquency. DCMA notification recommended. Ref: ${req.contractRef}.`
  }
  const issues: string[] = []
...
`
  ...
  groups.forEach(group => {
    ...
    html += `<table style="width:100%;border-collapse:collapse;font-size:6.5px;margin-bottom:12px;">
      <thead><tr>
        <th style="background:${group.color};color:#fff;padding:4px 6px;text-align:left;">ID</th>
        <th style="background:${group.color};color:#fff;padding:4px 6px;text-align:left;">Title</th>
        <th style="background:${group.color};color:#fff;padding:4px 6px;text-align:left;">DI Number</th>
        <th style="background:${group.color};color:#fff;padding:4px 6px;text-align:left;">Due</th>
        <th style="background:${group.color};color:#fff;padding:4px 6px;text-align:left;">Submitted</th>
        <th style="background:${group.color};color:#fff;padding:4px 6px;text-align:left;">RCVD</th>
        <th style="background:${group.color};color:#fff;padding:4px 6px;text-align:left;">Days</th>
        <th style="background:${group.color};color:#fff;padding:4px 6px;text-align:left;">Seal</th>
        <th style="background:${group.color};color:#fff;padding:4px 6px;text-align:left;">Notes / AI Remarks</th>
      </tr></thead><tbody>`
    ...
    group.rows.forEach(row => {
      ...
      html += `<tr>
        <td style="background:${bgColor};padding:5px 6px;border-bottom:1px solid #eee;">${row.id}</td>
        <td style="background:${bgColor};padding:5px 6px;border-bottom:1px solid #eee;">${row.title}</td>
        <td style="background:${bgColor};padding:5px 6px;border-bottom:1px solid #eee;">${row.diNumber}</td>
        <td style="background:${bgColor};padding:5px 6px;border-bottom:1px solid #eee;">${row.contractDueFinish}</td>
        <td style="background:${bgColor};padding:5px 6px;border-bottom:1px solid #eee;">${row.actualSubmissionDate || '—'}</td>
        <td style="background:${bgColor};padding:5px 6px;border-bottom:1px solid #eee;">${row.received}</td>
        <td style="background:${bgColor};padding:5px 6px;border-bottom:1px solid #eee;">${row.calendarDaysToReview !== null ? row.calendarDaysToReview : '—'}</td>
        <td style="background:${bgColor};padding:5px 6px;border-bottom:1px solid #eee;">${anchors[row.id] ? 'Sealed' : '—'}</td>
        <td style="background:${bgColor};padding:5px 6px;border-bottom:1px solid #eee;">${notes}</td>
      </tr>`
    })
    html += `</tbody></table>`
  })
  ...
  html += `<table style="width:100%;border-collapse:collapse;font-size:7px;margin-bottom:12px;border:1px solid #ddd;">
    <thead><tr>
      <th style="background:${ACCENT};color:#fff;padding:6px 8px;text-align:left;">DRL Category</th>
      <th style="background:${ACCENT};color:#fff;padding:6px 8px;text-align:center;">Shipbuilder / Contractor</th>
      <th style="background:${ACCENT};color:#fff;padding:6px 8px;text-align:center;">Ship Design Manager (SDM)</th>
      <th style="background:${ACCENT};color:#fff;padding:6px 8px;text-align:center;">Lead Reviewer</th>
      <th style="background:${ACCENT};color:#fff;padding:6px 8px;text-align:center;">Program Manager</th>
      <th style="background:${ACCENT};color:#fff;padding:6px 8px;text-align:center;">Quality Assurance</th>
    </tr></thead><tbody>`
  raciData.forEach((row, i) => {
    const bg = i % 2 === 0 ? '#fff' : LIGHT_BG
    html += `<tr>
      <td style="background:${bg};padding:5px 8px;font-weight:600;border-bottom:1px solid #eee;">${row[0]}</td>`
    for (let c = 1; c < row.length; c++) {
      const val = row[c]
      const color = val === 'R' || val.startsWith('R ') ? ACCENT : val === 'A' ? GREEN : TEXT
      const fw = val === 'R' || val.startsWith('R ') || val === 'A' ? '700' : '400'
      html += `<td style="background:${bg};padding:5px 8px;text-align:center;color:${color};font-weight:${fw};border-bottom:1px solid #eee;">${val}</td>`
    }
    html += `</tr>`
  })
  html += `</tbody></table>
  ...
  html += `<table style="width:100%;border-collapse:collapse;font-size:7.5px;margin-bottom:14px;border:1px solid #ddd;">
    <thead><tr>
      <th style="background:${ACCENT};color:#fff;padding:6px 10px;text-align:left;">Metric</th>
      <th style="background:${ACCENT};color:#fff;padding:6px 10px;text-align:center;">Last Week</th>
      <th style="background:${ACCENT};color:#fff;padding:6px 10px;text-align:center;">This Week</th>
      <th style="background:${ACCENT};color:#fff;padding:6px 10px;text-align:center;">Delta</th>
    </tr></thead><tbody>`
  progressData.forEach((row, i) => {
    const bg = i % 2 === 0 ? '#fff' : LIGHT_BG
    const deltaColor = row[3].startsWith('+') || row[3].includes('improved') ? GREEN : row[3].startsWith('-') ? GREEN : TEXT
    html += `<tr>
      <td style="background:${bg};padding:5px 10px;font-weight:600;border-bottom:1px solid #eee;">${row[0]}</td>
      <td style="background:${bg};padding:5px 10px;text-align:center;border-bottom:1px solid #eee;">${row[1]}</td>
      <td style="background:${bg};padding:5px 10px;text-align:center;border-bottom:1px solid #eee;">${row[2]}</td>
      <td style="background:${bg};padding:5px 10px;text-align:center;color:${deltaColor};font-weight:700;border-bottom:1px solid #eee;">${row[3]}</td>
    </tr>`
  })
  html += `</tbody></table>`
  ...
  if (aiInsights && Object.keys(aiInsights).length > 0) {
    ...
    html += `<table style="width:100%;border-collapse:collapse;font-size:7px;margin-bottom:12px;border:1px solid #ddd;">
      <thead><tr>
        <th style="background:${ACCENT};color:#fff;padding:4px 8px;text-align:left;">DRL ID</th>
        <th style="background:${ACCENT};color:#fff;padding:4px 8px;text-align:left;">Title</th>
        <th style="background:${ACCENT};color:#fff;padding:4px 8px;text-align:left;">Priority</th>
        <th style="background:${ACCENT};color:#fff;padding:4px 8px;text-align:left;">Next Action</th>
        <th style="background:${ACCENT};color:#fff;padding:4px 8px;text-align:left;">Target Date</th>
      </tr></thead><tbody>`
    sortedInsights.forEach((ins, i) => {
      ...
      const rowBg = i % 2 === 0 ? '#fff' : LIGHT_BG
      html += `<tr>
        <td style="background:${rowBg};padding:4px 8px;border-bottom:1px solid #eee;">${ins.id}</td>
        <td style="background:${rowBg};padding:4px 8px;border-bottom:1px solid #eee;">${row?.title?.slice(0, 35) ?? ''}</td>
        <td style="background:${rowBg};padding:4px 8px;border-bottom:1px solid #eee;color:${pColor};font-weight:700;">${ins.priority}</td>
        <td style="background:${rowBg};padding:4px 8px;border-bottom:1px solid #eee;">${ins.nextActions[0]?.action?.slice(0, 60) ?? ''}</td>
        <td style="background:${rowBg};padding:4px 8px;border-bottom:1px solid #eee;">${ins.nextActions[0]?.dueDate ?? ''}</td>
      </tr>`
    })
    html += `</tbody></table>`
    html += reportFooter(dateStr)
  ...
  if (recentEvents.length > 0) {
    html += `<table style="width:100%;border-collapse:collapse;font-size:6.5px;margin-bottom:12px;border:1px solid #ddd;">
      <thead><tr>
        <th style="background:${ACCENT};color:#fff;padding:4px 8px;text-align:left;">Date</th>
        <th style="background:${ACCENT};color:#fff;padding:4px 8px;text-align:left;">DRL</th>
        <th style="background:${ACCENT};color:#fff;padding:4px 8px;text-align:left;">Event</th>
        <th style="background:${ACCENT};color:#fff;padding:4px 8px;text-align:left;">Description</th>
        <th style="background:${ACCENT};color:#fff;padding:4px 8px;text-align:left;">AI Summary</th>
      </tr></thead><tbody>`
    recentEvents.forEach((evt, i) => {
      const evtBg = i % 2 === 0 ? '#fff' : LIGHT_BG
      html += `<tr>
        <td style="background:${evtBg};padding:3px 8px;border-bottom:1px solid #eee;">${new Date(evt.timestamp).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</td>
        <td style="background:${evtBg};padding:3px 8px;border-bottom:1px solid #eee;">${evt.rowId}</td>
        <td style="background:${evtBg};padding:3px 8px;border-bottom:1px solid #eee;">${evt.type}</td>
        <td style="background:${evtBg};padding:3px 8px;border-bottom:1px solid #eee;">${evt.description.slice(0, 80)}</td>
        <td style="background:${evtBg};padding:3px 8px;border-bottom:1px solid #eee;">${evt.aiSummary.slice(0, 60)}</td>
      </tr>`
    })
    html += `</tbody></table>`
  }
